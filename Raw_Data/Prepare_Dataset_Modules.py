...
def relabel_masks_in_folder(folder_path):
	...
	for filename in tqdm(os.listdir(folder_path), desc = f'Sequentially relabelling Masks', leave = True):
...
def check_data_sanity():
	...
	Raises:
		None
	"""
	base_dirs = ['Training', 'Validation', 'Augmented']

	for base_dir in base_dirs:
		images_dir = os.path.join('/home', 'ajinkya', 'Desktop', 'PyOrganoidAnalysis', 'Dataset', base_dir, 'Images')
		masks_dir = os.path.join('/home', 'ajinkya', 'Desktop', 'PyOrganoidAnalysis', 'Dataset', base_dir, 'Masks')
...
def validate_and_count_images():
	...
	"""
	base_folders = ['Training', 'Validation', 'Augmented']
	...
	for base in base_folders:
		for img_type, requirements in types_and_requirements.items():
			directory = os.path.join('/home', 'ajinkya', 'Desktop', 'PyOrganoidAnalysis', 'Dataset', base, img_type)
	...
	for filename in tqdm(sorted(os.listdir(folder_path)), desc='Calculating statistics on Masks', leave=True):
		if filename.endswith(".tiff") or filename.endswith(".tif"):
			...
			with Image.open(file_path) as img:
				...
				if filename.startswith("Test_Human_") or filename.startswith("Human_"):
					total_human_blobs += blob_count
					human_file_count += 1
				elif filename.startswith("Test_Mouse_") or filename.startswith("Mouse_"):
